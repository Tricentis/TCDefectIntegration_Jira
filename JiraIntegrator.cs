...
using RestProxy;
using TCDefectIntegration.BrowserWindow;
using System.IO;
...
namespace TCDefectIntegration
{
    public class JiraIntegrator : Integrator
    {
        ...
        private JiraRestManager restmanager;
        ...
        public override string CreateDefect(Dictionary<string, string> defectInfos)
        {
            ...
            try
            {
                ...
                // manual creation & restful mode
                if (Settings.Default.UseManualDefectCreationWindow || Settings.Default.UseRestfulConnection)
                {
                    string url = GetDefectIntegrationSetting("JiraCreateIssueURL", defectInfos);
                    // open browser
                    BrowserForm browser = new BrowserForm(url,Settings.Default.User,Settings.Default.Password,Settings.Default.AutomateLogin);
                    browser.ShowDialog();

                    defectId = browser.issueId;
                    browser.Dispose();
                ...
                else
                {
                    ...
                    {
                        customfieldId = "customfield_" + property.id,
                        values = property.value.Split(',')
                    }).ToArray();
                    issue = jira.createIssue(jiraToken, issue);
                    Disconnect();

                    defectId = issue != null ? issue.key : string.Empty;
                }

                if (Settings.Default.UseRestfulConnection)
                {
                    // attach attachment
                    string key = defectInfos.Keys.Where(k => k.Contains("Attachment")).FirstOrDefault();
                    if (!string.IsNullOrEmpty(key))
                    {
                        string filePath = defectInfos[key];
                        string fileType = getFileEnding(filePath);
                        //get extension
                        restmanager.addAttachment(defectId, filePath, "Attachment from Tosca." + fileType, "image/" + fileType);
                    }
                }
            }
            catch (Exception e)
            {
                MessageBox.Show(
                        "Jira submit failed with the following reason: " + "\r\n" + e.Message + e.StackTrace,
                        "Error occured",
        ...
        public override Dictionary<string, string> GetStatesForDefects(List<string> defectIds)
        {
            ...
            RemoteStatus[] remoteStatuses = null;
            Dictionary<string, string> statusNames = new Dictionary<string, string>();

            if (Settings.Default.UseRestfulConnection.Equals(false))
            {
                remoteStatuses = jira.getStatuses(jiraToken);
                statusNames = remoteStatuses.ToDictionary(remoteStatus => remoteStatus.id, remoteStatus => remoteStatus.name);
            }
            ...
            foreach (string defectId in defectIds)
            {
                try
                {
                    if (Settings.Default.UseRestfulConnection)
                    {
                        result.Add(defectId, restmanager.getIssueStatus(defectId));
                    }
                    else
                    {
        ...
        private bool Connect()
        {
            ...
            while (!loginWasSuccessful)
            {
                ...
                try
                {
                    ...
                    if (Settings.Default.UseRestfulConnection)
                    {
                        restmanager = new JiraRestManager(Settings.Default.User,
                                                                         Settings.Default.Password,
                                                                         Settings.Default.RestfulURL,
                                                                         InitProxy());

                        if (restmanager.checkCredentials())
                        {
